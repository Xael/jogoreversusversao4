...
// --- Funções para a visualização DENTRO do jogo ---
                ...
                ${sortedLeaderboard.map((player, index) => `
                    <tr>
                        ...
                        <td class="tournament-player-cell">
                           <img src="${player.avatar_url || './aleatorio1.png'}" class="tournament-player-avatar" alt="Avatar">
...
function renderInGameMatches(schedule, currentRound) {
    if (!schedule) return '';
    const { userProfile } = getState();
    ...
    return `
        <div class="matches-grid">
            ${roundMatches.map(match => {
                ...
                let resultText = '';
                if (isFinished) {
                    if (match.result === 'draw') resultText = 'E'; // Empate
                    else if (match.winnerId === match.p1.id) resultText = 'V';
                    else resultText = 'V';
                }
                ...
                return `
                    <div class="match-card ${isMyMatch ? 'my-match' : ''} ${isFinished ? 'finished' : ''}">
                        <div class="match-player">
                             <img src="${match.p1.avatar_url || './aleatorio1.png'}" class="match-player-avatar" alt="Avatar">
                             <span>${getPlayerName(match.p1)}</span>
                        </div>
                        <div class="match-result">${isFinished ? resultText : 'vs'}</div>
                        <div class="match-player">
                             <img src="${match.p2.avatar_url || './aleatorio1.png'}" class="match-player-avatar" alt="Avatar">
...
export function renderInGameTournamentView() {
    const { tournamentState } = getState();
    if (!tournamentState || !dom.tournamentGameView) return;

    const leaderboardHTML = renderInGameLeaderboard(tournamentState.leaderboard);
    const matchesHTML = renderInGameMatches(tournamentState.schedule, tournamentState.currentRound);

    dom.tournamentGameView.innerHTML = `
        <div class="tournament-ingame-section">
            <h3 class="tournament-section-title">${t('tournament.leaderboard')}</h3>
            ${leaderboardHTML}
        </div>
        <div class="tournament-ingame-section">
            <h3 class="tournament-section-title">${t('tournament.current_round', { round: tournamentState.currentRound })}</h3>
            ${matchesHTML}
        </div>
    `;
}